...
int alturaArvore(No* root, int altura)
{
    if(root == NULL)
    {
        altura--;
    ...
    {
        int contEsq = alturaArvore(root->esq, altura + 1);
        int contDir = alturaArvore(root->dir, altura + 1);
        altura = (contEsq > contDir) ? contEsq : contDir;
    }
    return altura;
}
...
int main()
{
    ...
    printf("altura: %d", alturaArvore(a1->root, 0));
