...
bool vote(int rank, string name, int ranks[])
{
    for (int i = 0; i < candidate_count; i++)
    {
        // Aqui só corrigi strcpm para strcmp
...
void add_pairs(void)
{
    // Implementação correta segundo o algoritmo Tideman
    for (int i = 0; i < candidate_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
...
void sort_pairs(void)
{
    // Bubble sort baseado na força da vitória
...
bool creates_cycle(int winner, int loser)
{
    // Se voltamos ao início, ciclo detectado
    if (winner == loser)
    {
        return true;
    }
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        // Se loser -> i já está locked, continue o caminho recursivamente
        if (locked[loser][i])
        {
...
void print_winner(void)
{
    // O vencedor é quem não tem ninguém apontando para ele
    for (int i = 0; i < candidate_count; i++)
    {
        bool source = true;
