...
// Cria um novo no e aloca sua memoria
...
// Chama a insercao de forma recursiva buscando a folha ideal
No* inserirRec(Restaurante* x, No* i) {
    if (i == NULL) {
        i = novoNo(x);
    } else if (strcmp(x->nome, i->restaurante->nome) < 0) {
...
int pesquisarRec(char* nome, No* i, int* comparacoes) {
    if (i == NULL) {
        return 0;
    } else {
        (*comparacoes)++;
        if (strcmp(nome, i->restaurante->nome) == 0) {
            ...
            if (strcmp(nome, i->restaurante->nome) < 0) {
                printf("esq ");
                return pesquisarRec(nome, i->esq, comparacoes);
            } else {
                printf("dir ");
                return pesquisarRec(nome, i->dir, comparacoes);
            }
        }
    }
}
...
void pesquisar(char* nome, int* comparacoes) {
    printf("raiz ");
    int resp = pesquisarRec(nome, raiz, comparacoes);
    if (resp) {
        printf("SIM\n");
    } else {
        printf("NAO\n");
...
// Libera da memoria de forma recursiva os nos alocados na arvore
...
int main()
{
    ...
    while(scanf(" %[^\n]", nome) == 1)
    {
        // Remove o carriage return (\r) se existir no final da string para evitar problemas de leitura
        int len = strlen(nome);
    ...
    gerar_log("892665", "arvore_binaria", comparacoes, tempo);
