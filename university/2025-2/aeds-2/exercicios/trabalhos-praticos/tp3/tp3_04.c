...
// Constroi e ajusta o heap a partir de um no raiz
void construir(Restaurante** array, int tam, int i, int* comp, int* mov) {
    int maior = i;
    int esq = 2 * i + 1;
    int dir = 2 * i + 2;
    ...
    if (esq < tam) {
        (*comp)++;
        int cmp = comparar_datas(array[esq]->data_abertura, array[maior]->data_abertura);
        if (cmp == 0) {
            (*comp)++;
            cmp = strcmp(array[esq]->nome, array[maior]->nome);
        }
        if (cmp > 0) maior = esq;
    }
    ...
    if (dir < tam) {
        (*comp)++;
        int cmp = comparar_datas(array[dir]->data_abertura, array[maior]->data_abertura);
        if (cmp == 0) {
            (*comp)++;
            cmp = strcmp(array[dir]->nome, array[maior]->nome);
        }
        if (cmp > 0) maior = dir;
    }
    ...
    if (maior != i) {
        swap(array, i, maior);
        (*mov) += 3;
        construir(array, tam, maior, comp, mov);
    }
}

// Compara duas structs de Data retornando a diferenca cronologica
...
int main()
{
    ...
    // Construir max heap para os K primeiros
    for (int i = k / 2 - 1; i >= 0; i--) {
        construir(selecionados, k, i, &comparacoes, &movimentacoes);
    }

    // Para o resto, se for menor que a raiz, substitui e refaz o heap
    for (int i = k; i < total; i++) {
        comparacoes++;
        int cmp = comparar_datas(selecionados[i]->data_abertura, selecionados[0]->data_abertura);
        if (cmp == 0) {
            comparacoes++;
            cmp = strcmp(selecionados[i]->nome, selecionados[0]->nome);
        }
        if (cmp < 0) {
            swap(selecionados, i, 0);
            movimentacoes += 3;
            construir(selecionados, k, 0, &comparacoes, &movimentacoes);
        }
    }

    // Ordenar o heap
    for (int i = k - 1; i > 0; i--) {
        swap(selecionados, 0, i);
        movimentacoes += 3;
        construir(selecionados, i, 0, &comparacoes, &movimentacoes);
    }

    long tempo = (long)(((double)(clock() - inicio)) / CLOCKS_PER_SEC * 1000);


    // Imprime toda a extensao do array para visualizacao do resultado final
    ...
    gerar_log("892665", "heapsort_parcial", comparacoes, movimentacoes, tempo);
