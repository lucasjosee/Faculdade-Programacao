 ...
 * TP 04 - Q3 - Tabela Hash Direta com Reserva em C
 */
...
Celula tabela[TAM_TOTAL];
int prox_reserva = TAM_TAB;
...
int main()
{
    ...
    gerar_log("892665", "hash_reserva", comparacoes, tempo);

    free_colecao(c);
    return 0;
}

/*---- Funções da Hash com Reserva ----*/
...
void iniciar()
{
    ...
    {
        tabela[i].restaurante = NULL;
        tabela[i].ocupado = 0;
    }
    prox_reserva = TAM_TAB;
}
...
void inserir(Restaurante* r)
{
    ...
    else
    {
        ...
        {
            tabela[prox_reserva].restaurante = r;
            tabela[prox_reserva].ocupado = 1;
            prox_reserva++;
        }
        else
        {
...
void pesquisar(char nome[], int* comparacoes)
{
    ...
    (*comparacoes)++;
    if(strcmp(tabela[h].restaurante->nome, nome) == 0)
    {
        char buffer[MAX_BUFFER];
        formatar_restaurante(tabela[h].restaurante, buffer);
        printf("%d %s\n", h, buffer);
        return;
    }
    ...
    for(int i = TAM_TAB; i < TAM_TOTAL; i++)
    {
        ...
        {
            (*comparacoes)++;
            if(strcmp(tabela[i].restaurante->nome, nome) == 0)
            {
                char buffer[MAX_BUFFER];
                formatar_restaurante(tabela[i].restaurante, buffer);
                printf("%d %s\n", i, buffer);
                return;
            }
        }
