...
void counting_sort(Restaurante** selecionados, int tamanho, int* comparacoes, int* movimentacoes);
...
int main()
{
    ...
    counting_sort(selecionados, total, &comparacoes, &movimentacoes);
    ...
    gerar_log("892665", "countingsort", comparacoes, movimentacoes, tempo);
...
void counting_sort(Restaurante** selecionados, int tamanho, int* comparacoes, int* movimentacoes)
{
    //Counting sort nao faz comparacoes entre elementos
    *comparacoes = 0;

    //Encontra o maximo e minimo de capacidade
    int max_cap = selecionados[0]->capacidade;
    int min_cap = selecionados[0]->capacidade;

    for(int i = 1; i < tamanho; i++)
    {
        if(selecionados[i]->capacidade > max_cap)
            max_cap = selecionados[i]->capacidade;
        if(selecionados[i]->capacidade < min_cap)
            min_cap = selecionados[i]->capacidade;
    }

    int range = max_cap - min_cap + 1;
    int* count = (int*) calloc(range, sizeof(int));
    Restaurante** saida = (Restaurante**) malloc(tamanho * sizeof(Restaurante*));

    //Conta as ocorrencias de cada capacidade
    for(int i = 0; i < tamanho; i++)
        count[selecionados[i]->capacidade - min_cap]++;

    //Acumula as contagens
    for(int i = 1; i < range; i++)
        count[i] += count[i - 1];

    //Constroi o array de saida (de tras para frente para ser estavel)
    for(int i = tamanho - 1; i >= 0; i--)
    {
        saida[--count[selecionados[i]->capacidade - min_cap]] = selecionados[i];
        (*movimentacoes)++;
    }
