...
int particionar(Restaurante** selecionados, int esq, int dir, int* comparacoes, int* movimentacoes);
void quicksort(Restaurante** selecionados, int esq, int dir, int* comparacoes, int* movimentacoes);
...
int main()
{
    ...
    quicksort(selecionados, 0, total - 1, &comparacoes, &movimentacoes);
    ...
    gerar_log("892665", "quicksort", comparacoes, movimentacoes, tempo);
...
int particionar(Restaurante** selecionados, int esq, int dir, int* comparacoes, int* movimentacoes)
{
    Restaurante* pivo = selecionados[dir];
    int i = esq - 1;
    ...
    for(int j = esq; j < dir; j++)
    {
        (*comparacoes)++;
        int cmp;
        double diff = selecionados[j]->avaliacao - pivo->avaliacao;
        if(diff < 0)
            cmp = -1;
        else if(diff > 0)
            cmp = 1;
        ...
        {
            (*comparacoes)++;
            cmp = strcmp(selecionados[j]->nome, pivo->nome);
        }

        if(cmp < 0)
        {
            i++;
            swap(selecionados, i, j);
            (*movimentacoes) += 3;
        }
    }

    swap(selecionados, i + 1, dir);
    (*movimentacoes) += 3;
    return i + 1;
}
...
void quicksort(Restaurante** selecionados, int esq, int dir, int* comparacoes, int* movimentacoes)
{
    if(esq < dir)
    {
        int pivo = particionar(selecionados, esq, dir, comparacoes, movimentacoes);
        quicksort(selecionados, esq, pivo - 1, comparacoes, movimentacoes);
        quicksort(selecionados, pivo + 1, dir, comparacoes, movimentacoes);
    }
}
